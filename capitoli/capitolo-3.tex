...
Sono oggetti che operano all'interno di un server web (es. Tomcat) oppure un server per applicazioni (come nel mio caso, GlassFish) permettendo la creazione di applicazione web e l'elaborazione di richieste da parte di un client.
...
Nel mio caso ho sviluppato tre servlet:
\begin{itemize}
	\item DataServlet.java: contenente il servlet per l'inizializzazione  dei contenuti di listData.jsp e altre richieste di dati da parte degli script Angular per la paginazione dei risultati e la visualizzazione dei grafici;
	\item MatrixServlet.java: contenente il servlet per l'inizializzazione  dei contenuti di corrMatrix.jsp;
	\item TargetServlet.java: contenente il servlet per l'inizializzazione  dei contenuti di corrTarget.jsp e altre richieste di dati da parte degli script Angular per la visualizzazione dei grafici;
\end{itemize}

\subsubsection{JavaBeans}

Nel mio caso ho sviluppato tre JavaBeans:
\begin{itemize}
...
Oltre a queste classi, contenenti i modelli base degli oggetti, ho sviluppato altre tre classi per gestire alcuni metodi statici di servizio per semplificare il modello dei beans.
 
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.5\columnwidth]{jsp_architecture}
	\caption{Interazione fra JSP, Servlet e JavaBeans (Fonte: \href{https://goo.gl/bWRWty}{https://goo.gl/bWRWty})}
\end{figure}
