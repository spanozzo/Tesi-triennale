...
\subsection{Controllo dei versione}
...
\begin{itemize}
	...
	\item \textit{\textbf{Fork e Pull request}}: queste due operazioni permettono di clonare una \textit{repository} (tramite \textit{fork}) e, successivamente, proporre l’inclusione delle modifiche apportante all’interno del clone nella \textit{repository} originale.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{\gls{Bash}}: per effettuare l'analisi dei dati preliminare da parte del team \textins{big data} viene utilizzata la \textit{shell} di \textit{Linux} per eseguire i tool interessati (ad esempio Hive, Impala, Spark o per visualizzare il \textit{filesystem} HDFS).
\end{itemize}

%**************************************************************
\section{Tipo di clientela}
I clienti principali di I.T. Euro Consulting sono aziende nazionali e internazionali che lavorano nei seguenti settori:
\begin{itemize}
	...
	\item \textit{\textbf{Retail \& Fashion}}
...
Un ulteriore prova della propensione all'innovazione dell'azienda è la presenza di un team specifico ed in continua espansione per l'ambito \textit{big data}, cosa usuale all'estero ma ben più rara in Italia in aziende di medie dimensioni, nelle quali l'importanza dei dati e della potenzialità che essi possiedono non è ancora entrata nell'idea di business delle aziende.
