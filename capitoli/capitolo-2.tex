...
\begin{itemize}
	\item \textbf{Hue}\footcite{http://gethue.com/}: è un'applicazione che offre un'interfaccia e semplifica alcune operazioni, come le \textit{query} su Hadoop, di cui è utile la conoscenza di base ma scarsamente utilizzato in quanto risulta relativamente complesso per utilizzi semplici; \\\\
	\item \textbf{Cloudera Manager}\footcite{https://www.cloudera.com/products/product-components/cloudera-manager.html}: è una web app per la gestione dei servizi e dei tool di Hadoop. Permette di tenere sotto controllo il cluster, quindi le risorse disponibili e gli utenti collegati, e permette di gestire e visualizzare le attività dei tool attualmente in esecuzione e passati. È utilizzato principalmente dai \textit{system administrators} ma si rivela utile anche per il team \textit{big data} per un'analisi veloce del funzionamento \textit{real time} del sistema.
\end{itemize} 
\begin{figure}[!h] 
	\centering 
...
\end{figure}
Al termine dello studio di questi tool era richiesta lo comprensione e la successiva analisi del dataset oggetto del progetto, che verrà trattato in dettaglio nel capitolo successivo.
...
Per lo svolgimento dell'attività di stage, è stato deciso in comune accordo con il tutor che dovesse essere svolto presso la sede dell'azienda. Questo è stato concordato con lo scopo di favorire il dialogo tra me ed il tutor aziendale e di avere la possibilità di confrontarmi direttamente con programmatori ed analisti più esperti in caso di problematiche durante lo svolgimento delle attività di progettazione, analisi e sviluppo software.
Oltre a ciò, l’azienda richiedeva che, al termine di ogni settimana lavorativa, venisse compilato un rapporto di quelle che erano state le attività svolte durante tale settimana. Inoltre, nel corso del primo periodo, prettamente di formazione, l'azienda richiedeva un breve resoconto di quanto compreso, cosicchè potessi essere più cosciente delle tecnologie che avrei utilizzato in seguito e, in caso di dubbi, questi venissero risolti da personale esperto.
...
Al termine di tutte le attività, l'azienda richiedeva inoltre una breve presentazione di quanto svolto ad alcuni membri del management. Tale presentazione, esposta in forma verbale, sarebbe servita allo studente per illustrare ciò che aveva concluso tramite il suo progetto, elencando pro e contro della soluzione da lui trovata.
...
\begin{figure}[!h] 
	\centering 
	\includegraphics[width=0.5\columnwidth]{tecnologie}
	\caption{Logo delle principali tecnologie per lo sviluppo software considerate (da sinistra a destra, rispettivamente: Java EE, Angular, React, Redux)}
\end{figure}